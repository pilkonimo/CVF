...
class RandomFlip(BasicTransform2D):
    def build_random_variables(self):
        self._random_variables = {
    ...
    def apply_transform_to_image(self, image):
        ...
        if self.get_random_variable("flip_x"):
            image = np.flip(image, axis=0)
        if self.get_random_variable("flip_y"):
            image = np.flip(image, axis=1)

        return image


class RandomRotation(BasicTransform2D):
    def build_random_variables(self):
        self._random_variables = {
            "nb_rotations": np.random.randint(4)
        }

    def apply_transform_to_image(self, image):
        assert image.ndim == 2

        nb_rotations = self.get_random_variable("nb_rotations")
        if nb_rotations > 0:
            image = np.rot90(image, k=nb_rotations, axes=(0, 1))
...
class ToTorchTensor(BasicTransform2D):
    def apply_transform_to_image(self, image):
        ...
        tensor = torch.from_numpy(image[None, ...].copy())
