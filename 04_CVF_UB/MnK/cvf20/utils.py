...
def normalize_dataset():
    dir_path = './yeast_cells_dataset'
    gt, raw = [], []
    for i in range(18):
        raw_path = os.path.join(dir_path, "raw_{}.png".format(i))
        gt_path = os.path.join(dir_path, "gt_{}.png".format(i))
        gt.append(plt.imread(gt_path))
        raw.append(plt.imread(raw_path))
    gt = np.stack(gt)
    raw = np.stack(raw)
    ...
    dst_path = os.path.join(dir_path, 'dataset.hdf5')
